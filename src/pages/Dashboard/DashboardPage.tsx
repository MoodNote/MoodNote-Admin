...
export default function DashboardPage() {
	...
	const statCards: StatCardData[] = useMemo(
		() => [
			...
			{ label: "Total Entries", value: stats?.entries.total, icon: "📝" },
			{
				label: "Entries Today",
				value: stats?.entries.new.today,
				icon: "📅",
			},
			{ label: "Tracks", value: musicStats?.totals.tracks, icon: "🎵" },
			{ label: "Genres", value: musicStats?.totals.genres, icon: "🎧" },
		],
		[musicStats, stats],
	);
				...
				<section className="dashboard-panel">
					...
					<ul className="top-genres-list">
						{(musicStats?.topGenres.length ?? 0) === 0 ? (
							...
							musicStats?.topGenres.map((genre) => (
								<li
									key={genre.genreId}
